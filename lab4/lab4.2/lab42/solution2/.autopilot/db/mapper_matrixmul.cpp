...
using hls::sim::Byte;
extern "C" void matrixmul(Byte<1>*, Byte<1>*, Byte<2>*);
...
std::vector<Byte<2>> __xlx_res__tmp_vec;
for (size_t i = 0; i < 9; ++i){
__xlx_res__tmp_vec.push_back(((Byte<2>*)__xlx_apatb_param_res)[i]);
}
  int __xlx_size_param_res = 9;
  int __xlx_offset_param_res = 0;
  int __xlx_offset_byte_param_res = 0*2;
  // DUT call
  matrixmul(__xlx_a__tmp_vec.data(), __xlx_b__tmp_vec.data(), __xlx_res__tmp_vec.data());
...
for (size_t i = 0; i < __xlx_size_param_res; ++i) {
((Byte<2>*)__xlx_apatb_param_res)[i] = __xlx_res__tmp_vec[__xlx_offset_param_res+i];
}
}
